...
with col_status[1]:
    ...
    <div class="metric-card">
        ...
        <p style="color: #64748b; font-size: 0.75rem; margin-top: 0.5rem;">Nemotron 70B Instruct</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    with col_input:
        ...
        with st.container(border=True):
            ...
            stb = st.number_input("⚖️ Stability Index", value=1.0000, format="%.4f", min_value=0.0, max_value=2.0, step=0.01)
        ...
        if st.button("🚀 GENERATE FORECAST", key="inference_btn", use_container_width=True, type="primary"):
            if predictor:
                ...
                with col_m2:
                    ...
                    st.markdown(f"""
                    <div class="metric-card">
                        <p style="color: #a0aec0; font-size: 0.85rem;">⚠️ Risk Level</p>
                ...
                st.error("⚠️ Model file not available. Check backend service.")
    ...
    with col_viz:
        ...
        # Add peak zone
        fig.add_hrect(y0=0.24, y1=0.30, annotation_text="⚠️ Peak Zone", annotation_position="right",
...
with tab2:
    ...
    if uploaded_file and predictor:
        try:
            ...
            with col_stat2:
                st.metric("🏷️ Features", len(raw_data.columns), help="Input columns")
            with col_stat3:
                file_size = len(raw_data.memory_usage(deep=True).sum()) / 1024**2
                st.metric("💾 Size", f"{file_size:.2f} MB")
            with col_stat4:
                st.metric("⏱️ Rows/sec", f"{len(raw_data)//max(1, len(raw_data.columns))}", help="Data density")
        ...
        st.warning("⚠️ Model not loaded. Check your backend service.")
    else:
        st.markdown("""
    ...
    if "agent_result" in st.session_state:
        ...
        if result.get("final_plan"):
            ...
            with col_plan2:
                ...
                st.markdown(f"""
                <div class="metric-card">
                    <p style="color: #a0aec0; font-size: 0.85rem;">⚠️ Risk Assessment</p>
                    ...
                    with st.expander(f"{priority_icon} **{rec.get('action', 'Action')}** — {priority}"):
                        ...
                        with col_rec2:
                            if rec.get('implementation_timeline'):
                                st.markdown(f"""
                                <div class="glass-card">
                                <p style="color: #a0aec0; font-size: 0.85rem;">⏱️ Timeline</p>
...
with tab4:
    ...
    with col_weekly2:
        ...
        <div class="glass-card">
            ...
            <div style="margin-bottom: 1rem;">
                <p style="color: #a0aec0; font-size: 0.85rem;">🏖️ Weekend Average</p>
